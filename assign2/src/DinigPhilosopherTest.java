...
public class DinigPhilosopherTest
{
	...
	public void test1() throws InterruptedException
	{
		...
		/*
		 * If DEBUG is set to true, your implementation should print in the console the
		 * major events that occur during the simulation For instance, Philosopher_1 is
		 * THINKING, Philosopher_4 is EATING, Philosopher_3 is HUNGRY, Philosoper_2
		 * released Chopstick_3, Philosopher_1 picked-up Chopstick_1, Deadlock detected
		 * …
		 */
		dp.DEBUG = true;
		...
		{
			assertTrue("The average eating times may not be correct!",
					p.getAverageEatingTime() > 10 && p.getAverageEatingTime() < 990);
			assertTrue("The average thinking times may not be correct!",
					p.getAverageThinkingTime() > 10 && p.getAverageThinkingTime() < 990);

			if (p.getId() == 0)
			{
				assertTrue("The average thinking time may not be correct!",
						Math.floor(p.getAverageThinkingTime()) == 693);
				assertTrue("The average eating time may not be correct!", Math.floor(p.getAverageEatingTime()) == 412);
			} else if (p.getId() == 1)
			{
				assertTrue("The average thinking time may not be correct!",
						Math.floor(p.getAverageThinkingTime()) == 315);
				assertTrue("The average eating time may not be correct!", Math.floor(p.getAverageEatingTime()) == 636);
			} else if (p.getId() == 2)
			{
				assertTrue("The average thinking time may not be correct!",
						Math.floor(p.getAverageThinkingTime()) == 571);
				assertTrue("The average eating time may not be correct!", Math.floor(p.getAverageEatingTime()) == 427);
			} else if (p.getId() == 3)
			{
				assertTrue("The average thinking time may not be correct!",
						Math.floor(p.getAverageThinkingTime()) == 335);
				assertTrue("The average eating time may not be correct!", Math.floor(p.getAverageEatingTime()) == 633);
			} else if (p.getId() == 4)
			{
				assertTrue("The average thinking time may not be correct!",
						Math.floor(p.getAverageThinkingTime()) == 313);
	...
	public void test2() throws InterruptedException
	{
		 ...
		 * Philosophers, columns correspond to the Philosopher ID (PID), average think
		 * time (ATT), average eating time (AET), average hungry time (AHT), number of
		 * thinking turns(#TT), number of eating turns(#ET), and number of hungry
		 * turns(#HT).
		 */
